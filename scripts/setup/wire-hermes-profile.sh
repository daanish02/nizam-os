...
# ensure_env_decrypted — decrypts .env.enc → .env on fresh clone if plaintext is absent
...
# wire_profile — migrates a hermes named profile's files into nizam-os and replaces them with symlinks
wire_profile() {
    local name="$1"
    ...
    for dir in skills memories; do
        ...
        if [ -L "$hs" ]; then              # both linked — hermes side already points into nizam-os
            log_info "$name/$dir: already symlinked — skip"
        elif [ -d "$hs" ] && [ -d "$ns" ]; then  # both real dirs — merge hermes content into nizam-os
        ...
        elif [ -d "$hs" ]; then            # hermes-only — real dir in hermes, nothing in nizam-os yet
            mv "$hs" "$ns"
            ln -sf "$ns" "$hs"
            log_info "$name/$dir: moved to nizam-os, symlinked"
        elif [ -d "$ns" ]; then            # nizam-only — nizam-os has it, hermes runtime doesn't
            ln -sf "$ns" "$hs"
            log_info "$name/$dir: linked from nizam-os"
        else                               # neither exists — create empty dir in nizam-os and link
            mkdir -p "$ns"
            ln -sf "$ns" "$hs"
    ...
    # second pass: link nizam-os-only files not present in hermes runtime
    while IFS= read -r -d '' f; do
        local base; base=$(basename "$f")
        local hp="$h/$base"
...
if [ $# -eq 0 ]; then
    ...
    for d in "$HERMES_PROFILES"/*/; do
        wire_profile "$(basename "$d")" || true  # non-fatal — continue to next profile if this one fails
    done
else
    for name in "$@"; do
        wire_profile "$name" || true  # non-fatal — continue to next profile if this one fails
    done
fi
