...
| `knowledge` | Noor's domain | `knowledge-service` |
| `personal` | Ayah's domain | `personal-service` |
| `finance_personal` | Ayah's financial domain | `finance-service` |
| `finance_business` | Hala's domain | `finance-service` |
| `crm` | Omar's domain | `crm-service` |
| `analytics` | Mira's domain | `analytics-service` |
...
**Core entity:** `audit.log`
- Tracks: which schema, which table, which operation (INSERT/UPDATE/DELETE), which agent (actor), the affected row, before and after state, and when.
- `actor` is the Hermes profile name — every write is attributed to the agent that made it.
- Grafana role has SELECT-only access for dashboards.
...
**vault_index** — one row per note in `~/nizam-vault/commons/`. Tracks the note's file path, title, areas (multi-value controlled vocabulary), source type and URL, tags (free-form), status lifecycle (raw → processed → evergreen), confidence level, and content hash.
...
- `tags` is free-form — no enforcement. Used for supplementary metadata (people, projects, events).
...
**goals** — a desired outcome with an optional target date and a status lifecycle (active → completed / paused / abandoned).

**tasks** — a discrete unit of work. Has priority, energy level (effort required), due date, status (open/done), and optional link to a goal.
...
**accounts** — a financial account the owner holds. Has a name, currency, and type (bank, cash, credit, investment).

**transactions** — one row per logged transaction. Has amount (in sub-units), currency, account, category, merchant, date, direction (income/expense), and optional exchange rate if multicurrency.
...
**savings_funds** — a goal-based savings target (e.g. emergency fund, car fund). Has a name, target amount, currency, and optional deadline.
...
**Key invariants:**
- `finance_personal` is entirely separate from `finance_business` — separate role, separate connection, separate migration. A compromise of one does not expose the other.
...
- Reporting conversion uses exchange rates stored at transaction time, not live rates.
- **Zakat has no persistent tables.** `calculate_zakat` is a runtime calculation: finance-service fetches the current gold price, computes the nisab threshold from zakatable account balances, and returns the obligation. Hawl eligibility (354 consecutive days above nisab) is computed from transaction history. Nisab gold gram threshold is configurable via `NISAB_GOLD_GRAMS` env var (default: 85g).

---

## `finance_business` schema

**Core entities:** revenue, expenses, invoices, accounts (business), and reporting periods.
